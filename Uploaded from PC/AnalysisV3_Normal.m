...
% no more update data collection, it ended on 12 Nov
ac1 = readtable('Export_2021-10-05_14-03-52.xlsx');
ac2 = readtable('vercat_Export_2021-11-19_18-29-14.xlsx');
ac = vertcat(ac1, ac2);
...
outside = day_light_saving_shift(outside);
...
nextTo = day_light_saving_shift(nextTo);
...
main = day_light_saving_shift(main);
...
%past = ac_date(1); % let it be first date of collection
limit_date = datetime(2021, 11, 13, 0, 0, 0); % collection ended 12 Nov
limit_25C_transient = 27.5;
...
toDelete_peak = summary_peak_table.datetime >= limit_date;
summary_peak_table(toDelete_peak,:) = [];

%% Collecting air data (nextTo, outside) between 2 peaks -> then average them
j = 1;
air_data = zeros(height(summary_peak_table),8);
...
clean_air_data_table = array2table(clean_air_data, 'VariableNames', ...
    {'minute_diff','energy_diff','out_humid','out_temp','out_light','next_humid','next_temp','next_heatindex'});
...
toDelete = complete_peak_table.minute_diff > 100;
complete_peak_table(toDelete,:) = [];

filename = 'finalform/collected_complete_peak_table.csv';
...
while(true)
    ...
    if datetime(2021, 9, 29, 0, 0, 0) <= past && past <= datetime(2021, 9, 29, 23, 59, 59) || ...
            datetime(2021, 9, 18, 0, 0, 0) <= past && past <= datetime(2021, 9, 18, 14, 59, 59) || ...
            datetime(2021, 9, 30, 0, 0, 0) <= past && past <= datetime(2021, 9, 30, 12, 59, 59) || ...
            datetime(2021, 10, 2, 0, 0, 0) <= past && past <= datetime(2021, 10, 2, 12, 59, 59) || ...
            skip
        skip = false;
...
saveas(gcf,'Figures/stackedplot_25C.fig')

filename = 'finalform/collected_summary_air_data.csv';
